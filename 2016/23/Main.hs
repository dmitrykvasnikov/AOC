...
data Instruction = CPY Arg Arg | INC Arg | DEC Arg | JNZ Arg Arg | TGL Arg | MUL Arg Arg Arg deriving (Show, Eq, Ord)
...
instruction = CPY <$ string "cpy " <*> arg <* space <*> arg
          ...
          <|> TGL <$ string "tgl " <*> arg
          <|> MUL <$ string "mul " <*> arg <* space <*> arg <* space <*> arg
...
step pc
  | halt pc = pc
  | length (program pc) < ptr pc || ptr pc < 1 = pc { halt = True }
  | otherwise = case program pc !! (ptr pc - 1) of
      ...
      TGL reg -> pc { ptr = ptr pc + 1, program = update (program pc) (ptr pc + eval reg(mem pc)) }
      MUL (Reg x) (Reg y) (Reg z) ->
        let y' = eval (Reg y) (mem pc)
            z' = eval (Reg z) (mem pc)
        in pc { ptr = ptr pc + 1 , mem = M.adjust (const (z' * y')) x (mem pc)}
      _ -> pc { ptr = ptr pc + 1 }

update :: Program -> Int -> Program
update prog ind =
  case ind >= 1 && ind <= length prog of
    True -> take (ind - 1) prog ++ [inst] ++ drop ind prog
    False -> prog
  where inst = case prog !! (ind - 1) of
                INC a -> DEC a
                DEC a -> INC a
                TGL a -> INC a
                CPY a b -> JNZ a b
...
main = do
  ...
  putStrLn $ "Part 1: " <> show (part1 input [7,0,0,0])
  putStrLn $ "Part 2: " <> show (part1 input [12,0,0,0])
