...
data Instruction = CPY Arg Arg | INC Arg | DEC Arg | JNZ Arg Arg | OUT Arg deriving (Show, Eq, Ord)
type Memory = M.Map Register Int
type Program = [Instruction]
type Signal = String
data PC = PC { program :: Program, mem :: Memory, ptr :: Int , halt :: Bool, sig :: Signal, aoc :: Bool, start :: Int}
type Input = Program
...
step pc
  ...
  | not . isPrefixOf (sig pc) $ (concat . repeat $ "01") = pc { halt = True }
  | length (sig pc) == 20 = pc { halt = True, aoc = True }
  | otherwise = case program pc !! (ptr pc - 1) of
      ...
      OUT reg -> pc { ptr = ptr pc + 1, sig = sig pc ++ (show $ eval reg (mem pc)) }
...
part1 :: Input -> [Int] -> Int
part1 input m =   start . head . filter aoc . map (\a -> head . filter halt . iterate step $ PC { halt = False, ptr = 1, program = input, mem = M.fromList (zip [A,B,C,D] [a,0,0,0]), sig = "", aoc = False, start = a}) $ [0..]
...
main = do
  ...
  putStrLn $ "Part 1: " <> show (part1 input [99,0,0,0])
