...
evalGame (Game p b s t c) d = case t of
  ...
  B | bh boss' <=0 -> [c]
    | otherwise ->
      evalGame (Game { player = player'' { ph = ph player'' - max 1 (bd b - armor) }, boss = boss', spells = spells', turn = next t, cost = c }) d
  where
    spells' = M.filter (>0) $ subtract 1 <$> s
    boss' = foldr castBoss b $ M.keys s
    player' = if t == P && d  then p { ph = ph p - 1 } else p
...
part1 :: Int
part1 = head . sort . filter (/=(-1)) . evalGame  (Game (Player 500 50) (Boss 51 9) M.empty P 0) $ False

part2 :: Int
part2 = head . sort . filter (/=(-1)) . evalGame  (Game (Player 500 50) (Boss 51 9) M.empty P 0) $ True
